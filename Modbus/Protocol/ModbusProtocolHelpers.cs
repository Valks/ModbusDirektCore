using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ModbusDirekt.Modbus.Protocol
{
    public static class ModbusProtocolHelpers
    {
        /// <summary>
        /// Converts a String to 16 - Bit Registers
        /// </summary>
        /// <param name="registers">Register array received via Modbus</param>
        /// <returns>Converted String</returns>
        public static int[] ConvertStringToRegisters(string stringToConvert)
        {
            byte[] array = System.Text.Encoding.ASCII.GetBytes(stringToConvert);
            int[] returnarray = new int[stringToConvert.Length / 2 + stringToConvert.Length % 2];
            for (int i = 0; i < returnarray.Length; i++)
            {
                returnarray[i] = array[i * 2];
                if (i * 2 + 1 < array.Length)
                {
                    returnarray[i] = returnarray[i] | ((int)array[i * 2 + 1] << 8);
                }
            }
            return returnarray;
        }

        public static bool DetectValidModbusFrame(byte[] readBuffer, int length)
        {
            // minimum length 6 bytes
            if (length < 6) return false;

            //SlaveID correct
            if ((readBuffer[0] < 1) || (readBuffer[0] > 247)) return false;

            //CRC correct?
            (byte, byte) crc = CRC16(readBuffer, (ushort)(length - 2), 0);

            if (crc.Item1 != readBuffer[length - 2] || crc.Item2 != readBuffer[length - 1])
                return false;
            return true;
        }
        /// <summary>
        /// Modbus CRC16 checksum calculator.
        /// Starting at (index) and n bytes of the message.
        /// </summary>
        /// <param name="data"></param>
        /// <param name="nBytes"></param>
        /// <param name="index"></param>
        /// <returns>Returns a tuple of bytes, whith LSB in Item1 and the MSB in Item2</returns>
        public static (byte, byte) CRC16(byte[] data, int nBytes, int index)
        {
            return CRC16(data.Skip(index).Take(nBytes).ToArray());
        }

        /// <summary>
        /// Modbus CRC16 checksum calculator
        /// </summary>
        /// <param name="data">Message to calculate CRC upon</param>
        public static (byte,byte) CRC16(byte[] data)
        {
            int usDataLen = data.Length;
            byte uchCRCHi = 0xFF;
            byte uchCRCLo = 0xFF;
            int i = 0;
            int uIndex;

            while (usDataLen > 0)
            {
                usDataLen--;
                if ((i) < data.Length)
                {
                    uIndex = uchCRCLo ^ data[i];
                    uchCRCLo = (byte)(uchCRCHi ^ auchCRCHi[uIndex]);
                    uchCRCHi = auchCRCLo[uIndex];
                }
                i++;
            }
            var b = BitConverter.GetBytes((ushort)((ushort)uchCRCHi << 8 | uchCRCLo));
            return (b[0], b[1]);
        }

        /*Table of CRC values for high-order byte*/
        static readonly byte[] auchCRCHi = {
            
            };

        /*Table of CRC values for low-order byte*/
        static readonly byte[] auchCRCLo = {
            
            };
    }
}
